...
(function () {
  ...
  async function fetchMovies(search) {
    const url = `https://www.omdbapi.com/?t=${search}&apikey=d5ea69ea`;
    try {
  ...
  function addToSuggestionContainerDOM(data) {
    ...
    if (!isPresent && data.Title != undefined) {
      if (data.Poster == "N/A") {
        data.Poster = "./images/notfound.jpg";
      }
      ...
      movieCard.innerHTML = `
        ...
        <a href="movieinfo.html" >
          ...
          <div class="card-body text-start">
            ...
            <p class="card-text">
            <i class="fas fa-star fa-lg" style="color: #ecce09;">
                <span id="rating">&nbsp;${data.imdbRating}</span>
              </i>

              <button class="fav-btn">
                <i class="fas fa-heart add-fav style=color:#f50505" data-id="${data.Title}"></i>
              </button>
            </p>
          </div>
        </a>
  ...
  function addToFavDOM() {
    ...
    <div class="movie-card-details">
      <p class="movie-name mt-3 mb-0">
       <a href = "movieinfo.html" class="fav-movie-name" data-id="${movie.Title}">${movie.Title}<a> 
      </p>
      <small class="text-muted">${movie.Year}</small>
    </div>

    <div class="delete-btn my-4">
        <i class="fas fa-trash-alt" data-id=${movie.Title}  style="color: #ff0000;"></i>
    </div>
    `;
